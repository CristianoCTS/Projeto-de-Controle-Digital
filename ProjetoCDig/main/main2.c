...
float PI_control(float Th, float Target, float SATmax, float SATmin)
{
    float er = Target - Th;
    float xi_new = s_xi + g_pi_c * (er + s_er_prev);
    float Upi = NB * er + KI_GAIN * xi_new;
    bool clamping = ((er > 0.0f) && (Upi > SATmax)) || ((er < 0.0f) && (Upi < SATmin));
